...
// Endpoint para el formulario de contacto general
app.post('/send-contact-email', async (req, res) => {
    const { email, phone, subject, body } = req.body;

    if (!email || !subject) {
        return res.status(400).json({ error: 'Faltan correo electrónico o asunto.' });
    }

    const finalBody = body && body.trim() !== '' ? body : "Hola, me interesaron mucho tus diferentes obras y trabajos, ponte en contacto conmigo!";

    const sendPromises = [];

    // --- NOTIFICACIÓN AL ADMINISTRADOR ---
    const adminEmailPromise = resend.emails.send({
        from: `Contacto Web <${SENDER_DOMAIN}>`,
        to: [ADMIN_EMAIL],
        ...
        html: `
            ...
            <p>${finalBody}</p>
            <hr>
            <p>Responde a este correo para contactar al remitente.</p>
        `,
    });
    sendPromises.push(adminEmailPromise);
    ...
    try {
        const results = await Promise.all(sendPromises);
        res.status(200).json({ message: 'Correo de contacto procesado.', results: results });
    } catch (error) {
        console.error("Error del servidor (Contacto):", error);
        res.status(500).json({ error: 'Fallo interno al procesar el envío del contacto.' });
    }
});
